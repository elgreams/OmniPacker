...
// Translation tables live in src/i18n/<lang>.js (one file per language) so
// additions and drift are reviewable per-language instead of buried in this
// file. The t() lookup and en-fallback behavior are unchanged.
const translations = { en, es, fr, de, ru };
...
// flow.
const addJobToQueue = () => {
  // Validate the AppID up front: an empty or non-numeric value would queue a
  // job as "unknown" that only fails later with a confusing DD error.
  const appIdValue = appIdInput?.value?.trim() || "";
  if (!/^\d+$/.test(appIdValue)) {
